...
void handleWebSocketMessage(SharedContext *ctx, const String &message) {
  ...
  if (doc["page"] == "device") {
    ...
    if (action == "set_mode") {
      ...
      targetState = ctx->pumpManualState;
      xSemaphoreGive(ctx->mutexContext);

      // Sync to other web clients — bao gồm pump_mode và pump_controller
      // để browser cập nhật đúng trạng thái hiển thị
      Webserver_sendata(ctx, "{\"pump_state\":\"" + String(targetState ? "ON" : "OFF") +
                             "\",\"pump_mode\":\"MANUAL\",\"pump_controller\":\"MANUAL\"}");

      // Sync to CoreIOT attributes (best-effort)
      coreiot_publish_attribute(ctx, "pumpState", targetState);
      ...
      if (doc["value"].containsKey("time")) {
        ...
        if (sep > 0) {
          ...
          xSemaphoreTake(ctx->mutexContext, portMAX_DELAY);
          if (h >= 0 && h <= 23)
            ctx->pumpScheduleHour = h;
          if (m >= 0 && m <= 59)
            ctx->pumpScheduleMinute = m;
          xSemaphoreGive(ctx->mutexContext);
        }
      }
    }
    ...
    Save_info_File(ctx, WIFI_SSID_NEW, WIFI_PASS_NEW, CORE_IOT_TOKEN_NEW,
                   CORE_IOT_SERVER_NEW, CORE_IOT_PORT_NEW);
  }
}
