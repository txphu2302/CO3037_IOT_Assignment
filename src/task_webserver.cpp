...
static void connectWSV(SharedContext *ctx) {
  ...
  ctx->webSocket->onEvent([ctx](AsyncWebSocket *server,
  ...
  ctx->webServer->on("/styles.css", HTTP_GET,
  ...
  ctx->webServer->on(
      "/api/status", HTTP_GET, [ctx](AsyncWebServerRequest *request) {
        String mode = "UNKNOWN";
        ...
        const int wsClients =
            (ctx->webSocket) ? (int)ctx->webSocket->count() : 0;
        ...
        json += ",\"ap_ip\":\"" + apIp + "\"";
        json +=
            ",\"mqtt_connected\":" + String(mqttConnected ? "true" : "false");
  ...
  ctx->webServer->on(
      "/toggle-led", HTTP_GET, [ctx](AsyncWebServerRequest *request) {
  ...
  ctx->webServer->on(
      "/toggle-pump", HTTP_GET, [ctx](AsyncWebServerRequest *request) {
        bool target = false;
        xSemaphoreTake(ctx->mutexContext, portMAX_DELAY);
        ...
        // Sync to other web clients
        Webserver_sendata(ctx, "{\"pump_state\":\"" + String(target ? "ON" : "OFF") + "\", \"pump_mode\":\"MANUAL\", \"pump_controller\":\"MANUAL\"}");

        // Sync to CoreIOT attributes (best-effort)
        coreiot_publish_attribute(ctx, "pumpState", target);
        coreiot_publish_attribute(ctx, "modeState", true); // Báo CoreIoT chế độ MANUAL
      });

  // Kick off async scan (non-blocking, returns immediately)
  ctx->webServer->on(
      "/scan/start", HTTP_GET, [](AsyncWebServerRequest *request) {
  ...
  ctx->webServer->on(
      "/scan/result", HTTP_GET, [](AsyncWebServerRequest *request) {
